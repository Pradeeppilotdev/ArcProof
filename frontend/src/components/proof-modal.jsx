...
const STAGE_COLORS = ["#a1a1aa", "#5b8def", "#e8a33d", "#5b8def", "#3dd68c"];
        ...
        return (
          <div key={s.label} className="flex items-start">
            <div className="flex flex-col items-center gap-2" style={{ width: 84 }}>
              ...
              >
                {done ? (
                  <CheckCircleIcon className="w-4 h-4" style={{ color: "#3dd68c" }} />
                ) : active ? (
                  <div className="w-1.5 h-1.5 rounded-full bg-blue-500 animate-pulse" />
                ) : (
                  <div className="w-1.5 h-1.5 rounded-full" style={{ background: "#a1a1aa" }} />
                )}
              </div>
              <div className="text-center">
                <div className="text-[11px] font-semibold transition-colors duration-500" style={{ color: done ? "#3dd68c" : active ? "#ededef" : "#a1a1aa" }}>
                  {s.label}
                </div>
            ...
            {i < steps.length - 1 && (
              <div
                ...
                style={{ background: stage > i ? "#3dd68c" : "#27272a" }}
              />
            )}
          </div>
        );
